...
# gml_toolchain_{pre,post}_features are used to extend the toolchain features of the default cc_toolchain_config.
...
#	)
# Additionally, the attrs in GML_EXTRA_CC_CONFIG_ATTRS are available in the passed in ctx.
#

# gml_toolchain_pre_features are added to the command line before any of the default features.
def gml_toolchain_pre_features(ctx):
    features = []
    features += _custom_c_runtime_path_pre(ctx)
    return features

# gml_toolchain_post_features are added to the command line after all of the default features.
def gml_toolchain_post_features(ctx):
    """gml_toolchain_post_features are added to the command line after all of the default features.
...
GML_EXTRA_CC_CONFIG_ATTRS = dict(
