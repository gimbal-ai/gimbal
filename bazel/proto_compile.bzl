...
def _gml_proto_library(name, srcs, deps = [], **kwargs):
    """
    Generates proto definition. Includes well known protos.
    """
    if len(srcs) > 1:
        fail("Only one srcs value supported", "srcs")
    if not name.endswith("_gml_proto"):
        fail("Expected gml_proto_library name to end with '_gml_proto'.")
...
def _gml_cc_proto_library(name, proto, deps = [], **kwargs):
    if not name.endswith("_gml_cc_proto"):
        fail("Expected gml_cc_proto_library name to end with '_gml_cc_proto'.")
...
def _gml_go_proto_library(name, proto, importpath, deps = [], **kwargs):
    if not name.endswith("_gml_go_proto"):
        fail("Expected gml_go_proto_library name to end with '_gml_go_proto'.")
...
def gml_py_proto_library(name, proto, deps = [], **kwargs):
    if not name.endswith("_gml_py_proto"):
        fail("Expected gml_py_proto_library name to end with '_gml_py_proto'.")
...
def _gml_py_grpc_library(name, proto, deps = [], **kwargs):
    if not name.endswith("_gml_py_grpc"):
        fail("Expected gml_py_grpc_library name to end with '_gml_py_grpc'.")
...
gml_proto_library = _gml_proto_library
gml_cc_proto_library = _gml_cc_proto_library
gml_go_proto_library = _gml_go_proto_library
gml_py_grpc_library = _gml_py_grpc_library
