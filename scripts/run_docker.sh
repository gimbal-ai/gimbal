...
docker_image_base=us-docker.pkg.dev/gimlet-dev-infra-0/gimlet-dev-infra-public-docker-artifacts/dev_image_with_extras
os="$(uname)"
...
read -ra GML_RUN_DOCKER_EXTRA_ARGS <<<"${GML_RUN_DOCKER_EXTRA_ARGS}"
...
CLEANED_ARGS=()
CURRENT=""
for arg in "${GML_RUN_DOCKER_EXTRA_ARGS[@]}"; do
  if [[ $CURRENT ]]; then
    CURRENT="${CURRENT} ${arg}"
    CURRENT="${CURRENT//\'/}"
    if [[ $arg = *"'" ]]; then
...
configs=(
  ...
  -v "${workspace_root}:/gml/src/gimletlabs.ai/gimlet"
  # Use the host network so we can easily access build caches, etc.
  "--network=host"
)

if command -v gml >/dev/null; then
  # If "gml" exists, mount it into the container (in a well known path).
  gml_path=$(command -v gml)
  configs+=(-v "${gml_path}:/bin/gml")
fi
...
container_args=(
  --rm
  --hostname "gml-dev-docker-$(hostname)"
)
