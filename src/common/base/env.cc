...
void ChDirGimletRoot() {
  // If GIMLETROOT is set, and we are not running through bazel, run from ToT.
  const char* test_src_dir = std::getenv("TEST_SRCDIR");
  const char* gimletroot = std::getenv("GIMLETROOT");
  if (test_src_dir == nullptr && gimletroot != nullptr) {
    LOG(INFO) << absl::Substitute("Changing CWD to to GIMLETROOT [$0]", gimletroot);
    std::filesystem::current_path(gimletroot);
  }
}
