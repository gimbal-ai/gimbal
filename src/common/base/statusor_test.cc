...
TEST(StatusOr, ValueOr) {
  StatusOr<string> s(Status(gml::statuspb::UNKNOWN, "This is not OK"));
...
TEST(StatusOr, ConsumeValueOr) {
  ...
  {
    StatusOr<std::unique_ptr<int>> s(Status(gml::statuspb::UNKNOWN, "This is not OK"));
...
TEST(StatusOr, ValuesAndErrors) {
  ...
  s = StatusOr<string>(Status(gml::statuspb::UNKNOWN, "some error"));
  ASSERT_FALSE(s.ok());
...
TEST(StatusOr, DefaultCtor) {
  StatusOr<string> s;
  EXPECT_FALSE(s.ok());
  EXPECT_EQ(s.code(), gml::statuspb::UNKNOWN);
}
...
StatusOr<int> StatusOrTestFunc(int x) {
  if (x == 0) {
    return Status(gml::statuspb::INTERNAL, "badness");
  }
  return x + 1;
}
...
TEST(StatusOr, Macros) {
  ...
  EXPECT_FALSE(s.ok());
  EXPECT_EQ(gml::statuspb::INTERNAL, s.code());
}
