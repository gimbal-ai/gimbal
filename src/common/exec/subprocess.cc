...
std::filesystem::path MountNamespacePath(int pid) {
  return gml::system::ProcPidPath(pid, "ns", "mnt");
}
...
Status SubProcess::Start(const std::vector<std::string>& args, bool stderr_to_stdout) {
  ...
  GML_RETURN_IF_ERROR(pipe_.Open(O_NONBLOCK));
  ...
  if (child_pid_ == 0) {
    ...
    GML_ASSIGN_OR_RETURN(std::filesystem::path parent_exe_path, proc_parser.GetExePath(getpid()));
