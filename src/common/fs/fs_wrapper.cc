...
gml::StatusOr<std::filesystem::path> ReadSymlink(const std::filesystem::path& symlink) {
  std::error_code ec;
...
Status CreateSymlinkIfNotExists(const std::filesystem::path& target,
  ...
  GML_RETURN_IF_ERROR(fs::CreateDirectories(link.parent_path()));
  ...
  GML_ASSIGN_OR_RETURN(std::filesystem::path actual_target, fs::ReadSymlink(link));
...
StatusOr<std::filesystem::path> GetChildRelPath(const std::filesystem::path& child,
  ...
  GML_ASSIGN_OR_RETURN(std::filesystem::path res, Relative(child, parent));
