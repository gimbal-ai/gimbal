...
StatusOr<int64_t> ProcParser::GetPIDStartTimeTicks(int32_t pid) const {
  return ::gml::system::GetPIDStartTimeTicks(ProcPidPath(pid));
}
...
Status ProcParser::ReadProcPIDFDLink(int32_t pid, int32_t fd, std::string* out) const {
  ...
  GML_ASSIGN_OR_RETURN(std::filesystem::path link, fs::ReadSymlink(fpath));
  *out = std::move(link);
  return Status::OK();
}
...
Status ProcParser::ReadUIDs(int32_t pid, ProcUIDs* uids) const {
  ...
  GML_ASSIGN_OR_RETURN(std::string content, gml::ReadFileToString(proc_pid_status_path));
